	...
	class estudiantesController
	{
		...
		public function agregar(){
			if (!$_POST) {
				...
				$formatos = array("image/jpeg", "image/png", "image/gif", "image/jpg");
				$limite = 700;
				if (in_array($_FILES['imagen']['type'], $formatos) && $_FILES['imagen']['size'] <= $limite*1024) {
					...
					$this->estudiante->set("fecha", $fecha);
					$this->estudiante->add();
		...
		public function editar($id){
			if (!$_POST) {
				...
				if ($_POST['id_seccion'] == 0) {
					...
					$this->estudiante->set("edad", $_POST['edad']);
					$this->estudiante->set("promedio", $_POST['promedio']);
					$this->estudiante->set("id_seccion", $_POST['id_seccion_old']);
